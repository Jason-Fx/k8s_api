...
class podlog:
    @staticmethod
    def get_pods_log(podname,jobname,previous=None,type=None,tailline=None , namespaceUser = 'default'):
        print(previous)
        if int(previous)==0:
            ##True old
            a=True
        ...
        try:
            if type == "test":
                api_response = v1client.read_namespaced_pod_log(podname, namespaceUser, tail_lines=tailline, #limit_bytes=limit_bytes,
                                                                previous=a,pretty=pretty # since_seconds=since_seconds,
                                                                # timestamps=timestamps
                                                                )
            if type == "product":
                api_response = v1client.read_namespaced_pod_log(podname, namespaceUser, container=jobname, tail_lines=tailline,#limit_bytes=limit_bytes,
                                                                previous=a,pretty=pretty #since_seconds=since_seconds,
                                                                # timestamps=timestamps
                                                                )
